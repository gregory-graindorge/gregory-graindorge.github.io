...
Fuse was very interesting. It requires to make a good enumeration, but also to a have keen eye for details and a good understanding of what you're doing overall. The root part is pretty straightforward once you know how to use the exploit properly. Very fun box!
...
As you can see we have a bunch of ports open and the full scan reveals morein informations : 
...
Every user is valid users, but I can't go further with Kerberos because I don't have any password for these users.
...
I should've tried directly the password Fabricorp01 but for some reasons it didn't pop in my head so I tried different password lists but nothing worked. I decided to create a custom list based on the domain name (fabricorp).
...
Interestingly enough, if we use the password <strong>Fabricorp01</strong>, we get the message <strong><cite>STATUS_PASSWORD_MUST_CHANGE</cite></strong>.
...
Some kind of process resets all the passwords after one or two minutes, so I guess we just need to be quick to connect to smb. Also, we need to put a different password every time we change it because it keeps track of the old passwords.
