...
Bucket is one of the best box I've ever made on Hackthebox. Very challenging but in a way that forces you to learn more really understand every step that you make. There is no easy trick, you just need to enumerate and read the doc to find the misconfiguration inside the box. Very nice box !
...
Right away, we can notice that the domain name which is `bucket.htb`. I'm gonna put that in my `/etc/hosts` file and then move on to the website. Also I'm gonna take a note of the <strong><cite>Apache</cite></strong> version (2.4.41).

## Foothold / User
The website is only one html page.

I started `nikto` and `gobuster` targeting that page. Also I like to poke arround with <strong><cite>Firefox</cite></strong> to enumerate websites. What I've noticed here is the path that is requested to get the images `http://s3.bucket.htb/adserver/images/`.
